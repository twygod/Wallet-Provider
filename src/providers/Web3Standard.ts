...
class Web3Standard extends ProviderEventRegistry implements Provider {
   ...
   */
  async setProvider(provider: any, pkgInstance: any = null) {
    ...
    this._provider.provider.autoRefreshOnNetworkChange = false;

    if (pkgInstance != null) {
      this._provider.provider.__proto__.providerPackage = pkgInstance;
    }
  ...
  handlerEventLiteners() {
    //on connect
    this._provider.provider.on("connect", async (chainIdObj: any) => {
     ...
     * disconnect
     */
    this._provider.provider.on("disconnect", (err: any) => {
      this._onDisconnectCallback(err);
    });

    this._provider.provider.on("error", (error) => {
      this._onErrorCallback(error);
    });

    this._provider.provider.on("chainChanged", async (chainId) => {
      this._accounts = await this.getAccounts();
      this._onChainChangedCallback(chainId);
    });

    this._provider.provider.on(
      "accountsChanged",
    ...
    this._provider.provider.on("message", (message: string) => {
  ...
  async connect(): Promise<Status> {
    if (!this.isSupported()) {
      return Status.error("wallet_not_found").setCode(
        NetworkCodes.wallet_not_found
      );
    }

    //lets request access
    try {
      if (typeof this._provider.provider["enable"] == "function") {
        this._accounts = await this._provider.provider.enable();
      } else {
        this._accounts = await this.getAccounts();
      }
  ...
  async getAccounts(): Promise<Array<string>> {
    return new Promise((resolve, reject) => {
      this._provider.provider.sendAsync(
        {
  ...
  isConnected(): boolean {
    return (
      this._provider.provider.connected || this._provider.provider.isConnected()
    );
  }
  ...
  async disconnect(): Promise<Status> {
    this._provider.provider.disconnect();
